...
exports.register = async (req, res, next) => {
  ...
  try {
    ...
    const isFirstUser = (await User.countDocuments({})) === 0;  // First user ko automatically Admin banana
    ...
    const verificationToken = crypto.randomBytes(20).toString('hex'); // Iska use email verification ke liye hoga.
    const verificationTokenExpiry = Date.now() + 24 * 3600 * 1000; // 24 hours

    user = await User.create({ // Ab actual account create hota hai:
      fullName,
      email,
      password,
      avatar,
    ...
    await AuditLog.create({//Ye user register hua hai. Iska log database mein save ho jayega.
...
// Logout
// User ko logout karna by removing their stored refresh token from the database.
exports.logout = async (req, res, next) => {
  try {
    const user = await User.findById(req.user.id);
    if (user) {
      user.refreshToken = undefined; // Refresh token ko remove kar diya, matlab user logout ho gaya.
      await user.save();
...
//Access Token expire hone par new tokens banana
...
///Email verify karna + user ko automatically login kar dena
exports.verifyEmail = async (req, res, next) => {
  ...
  try {
    ...
    // ---> NAYA LOGIC: Token generate karein aur save karein <---
    const accessToken = generateAccessToken(user);
    const refreshToken = generateRefreshToken(user);
    user.refreshToken = refreshToken;
    ...
    await AuditLog.create({
      ...
      performedBy: user._id,
      details: 'User email verified successfully and auto-logged in.',
      ipAddress: req.ip
    });

    // ---> NAYA LOGIC: Response mein token bhejein <---
    res.status(200).json({
      success: true,
...
// Forgot Password-> forgotPassword() user ke email ko verify karta hai, ek secure temporary reset token generate karta hai, uska hash database mein 10-minute expiry ke saath save karta hai, audit log banata hai, aur development mein reset token frontend ko return karta hai.
//
...
// Reset Password
// User ke reset token ko verify karke, uska old password replace karke new password save karna.
