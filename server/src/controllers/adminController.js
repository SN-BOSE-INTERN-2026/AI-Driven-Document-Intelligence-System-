...
exports.getAnalytics = async (req, res, next) => {
  try {
    ...
    const popularCategories = categoryStats.map(item => ({ // converts MongoDB's result into a frontend-friendly format:
      name: item._id,
      value: item.count
    }));
    ...
    const popularTypes = typeStats.map(item => ({// Popular file types like pdf,docs
...
// Get system logs (AuditLog) ->Admin dashboard ke liye database se system ke activity/audit logs nikal kar frontend ko bhejna.
...
// Get all users (Admin view) ->Admin dashboard mein all users list ko display karna.
exports.getUsers = async (req, res, next) => {
  try {
    const users = await User.find({}).select('-password -refreshToken').sort({ createdAt: -1 });//Password aur refreshToken mat bhejo frontend pe
    res.status(200).json({
      success: true,
      count: users.length,
      users
...
//
//Admin kisi user ka role user se admin ya admin se user change kar sake.
exports.updateUserRole = async (req, res, next) => {
  ...
  try {
    const user = await User.findById(req.params.id); // jis user ko change karna hai
    if (!user) {
      return res.status(404).json({ success: false, message: 'User not found' });
    }

    // Prevent removing own admin privileges
    if (user._id.toString() === req.user._id.toString() && role !== 'admin') { //Admin khud ka admin role remove nahi kar sakta. nhi toh kon admin bachega
    ...
    await AuditLog.create({
      ...
      performedBy: req.user._id, // jo currently logged-in user/admin hai.
