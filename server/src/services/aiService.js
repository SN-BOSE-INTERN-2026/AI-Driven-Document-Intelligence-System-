  ...
  if (openai && !isGemini) {
    ...
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${geminiApiKey}`, {
        ...
        body: JSON.stringify({
          systemInstruction: { parts: [{ text: `You are an AI document summarization engine. Analyze the text and return a JSON object with keys:\n- "short": A 1-2 sentence high-level overview.\n- "detailed": A comprehensive 1-2 paragraph description.\n- "bulletPoints": A list of 3-6 key takeaways.` }] },
      ...
      console.warn('Gemini summarization failed, falling back to heuristics:', error.message);
    }
  }
...
const answerQuestion = async (question, chunks) => {
  ...
  if (openai && !isGemini) {
    ...
    try {
      ...
      console.warn('Gemini completions failed, falling back to local retrieval:', error.message);
    }
  }
